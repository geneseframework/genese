...
import { TreeFolderService } from '../tree/tree-folder.service';
...
export class TreeFolderReportService {
    ...
    generateReport(): void {
        ...
        const reportTemplate = eol.auto(fs.readFileSync(`${Options.pathGeneseNodeJs}/src/complexity/old/templates/handlebars/folder-report.handlebars`, 'utf-8'));
    ...
    private registerPartial(partialName: string, filename: string): void {
        const partial = eol.auto(fs.readFileSync(`${Options.pathGeneseNodeJs}/src/complexity/old/templates/handlebars/${filename}.handlebars`, 'utf-8'));
