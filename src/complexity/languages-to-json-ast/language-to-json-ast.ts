...
export class LanguageToJsonAst {
    ...
    static start(pathToAnalyze: string, language?: Language): void {
        ...
        switch (language) {
            case Language.TS:
            case Language.JAVA:
                jsonAst = LanguageToJsonAst.generateFromFiles(
                    pathToAnalyze,
                    language
                );
                break;
    ...
    private static generateFromAllFiles(
     ...
     */
    private static generateFromFiles(
        pathToAnalyze: string,
        language: Language
    ): JsonAstInterface {
        ...
        let astFolder = new InitGenerationService().generateAll(
            pathToAnalyze,
            language
        ).astFolder as any;
