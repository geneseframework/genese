...
export class AstFolderReportService {
    ...
    getFoldersArray(astFolder: AstFolder): RowFolderReport[] {
        ...
        if (
            getPathWithSlash(this.astFolder.path) !==
            getPathWithSlash(Options.pathFolderToAnalyze)
        ) {
    ...
    getSubfoldersArray(
        ...
        for (const subfolder of astFolder.children) {
            if (subfolder.relativePath !== "") {
                const ROUTE_FROM_CURRENT_FOLDER = this.astFolderService.getRouteFromFolderToSubFolder(
                    this.astFolder,
                    subfolder
                );
                const subfolderReport: RowFolderReport = {
                    complexitiesByStatus:
                        subfolder.stats?.numberOfMethodsByStatus,
            ...
            if (!isSubfolder) {
                report = report.concat(
                    this.getSubfoldersArray(subfolder, true)
                );
            }
        }
     ...
     * @param astFolder    // The AstFolder to analyse
     */
    getMethodsArraySortedByDecreasingCognitiveCpx(
        astFolder: AstFolder
    ): RowFileReport[] {
     ...
     */
    sortByDecreasingCognitiveCpx(
        methodsReport: MethodReport[]
    ): MethodReport[] {
    ...
    private writeReport() {
        ...
        const PATH_OUT_DIR = constructLink(Options.pathOutDir);
        const RELATIVE_PATH = constructLink(this.astFolder.relativePath);
        const pathReport = `${deleteLastSlash(PATH_OUT_DIR)}/${deleteLastSlash(
            RELATIVE_PATH
        )}/folder-report.html`;
